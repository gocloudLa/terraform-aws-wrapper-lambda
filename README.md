...
- **⚠️ REST APIGateway shared stage:** Each `terraform-aws-wrapper-lambda` call that attaches `apigateway_rest` triggers to the same `rest_api_name` owns its own `aws_api_gateway_deployment`/`aws_api_gateway_stage`. If two independent workloads target the same REST API and stage name, use distinct `stage_name` values (or coordinate a single owner) to avoid one apply's deployment overwriting another's.
